...
ga('create', 'UA-47578991-1', 'java8.org');
ga('send', 'pageview');
	...
	jQuery(document)
			.ready(
					function($) {
						...
						var mce_validator = $("#mc-embedded-subscribe-form")
								.validate(options);
						$("#mc-embedded-subscribe-form").unbind('submit');

						options = {
							url : 'http://ohmdb.us3.list-manage.com/subscribe/post-json?u=c2c27aa25fdef31d9513d6124&id=ea2d68d2fd&c=?',
							type : 'GET',
							dataType : 'json',
							contentType : "application/json; charset=utf-8",
							beforeSubmit : function() {
								$('#mce_tmp_error_msg').remove();
								$('.datefield', '#mc_embed_signup')
										.each(
												function() {
													var txt = 'filled';
															...
															.each(
																	function() {
																		if (fields[0].value.length != 3
																				|| fields[1].value.length != 3
																				|| fields[2].value.length != 4) {
																			this.value = '';
																		} else {
																			this.value = 'filled';
																		}
